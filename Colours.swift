...
public extension Color {
    ...
    func hsba() -> (h: CGFloat, s: CGFloat, b: CGFloat, a: CGFloat) {
        ...
        if self.responds(to: #selector(getHue(_:saturation:brightness:alpha:))) && self.cgColor.numberOfComponents == 4 {
    ...
    func CIE_LAB() -> (l: CGFloat, a: CGFloat, b: CGFloat, alpha: CGFloat) {
        ...
        let deltaF: TransformBlock = { f in
            // Workaround on "Expression was too complex..." Swift error
            let k1: CGFloat = pow(6.0/29.0, 3.0)
            let k2: CGFloat = 1.0/3.0 * pow(29.0/6.0, 2.0)

            let transformation = f > k1 ? pow(f, 1.0/3.0) : k2 * f + 4.0/29.0
